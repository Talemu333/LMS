...
import {
  Bell, BookOpen, ChevronRight, CirclePlus, ClipboardCheck, GraduationCap,
  House, LogOut, Menu, MessageCircle, X, Layers, UserRound
} from 'lucide-react';
import './styles.css';
    ...
    <div className="auth">
      <div className="auth-box">
        ...
        <div className="role-switch">
          ...
          <button className={role === 'instructor' ? 'active' : ''} onClick={() => { setRole('instructor'); resetForm(); }} disabled={mode === 'register'}>Instructor</button>
        </div>

        {mode === 'login' ? (
          <form onSubmit={submit}>
            <h2>Welcome back</h2>
            <p className="muted">Sign in to continue to your learning space.</p>
            ...
            {role === 'student' && <button type="button" className="btn btn-light full" onClick={() => { resetForm(); setMode('register'); }}>Create Account</button>}
          ...
          <form onSubmit={submit}>
            ...
            <p className="muted">Create your ELES learning account.</p>
...
function Field({ label, type = 'text', value, onChange, placeholder }) {
  return <div className="form-group"><label>{label}</label><input required type={type} value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)} /></div>;
}
  ...
  return (
    <div className="app">
      <header className="topbar">
        <div className="brand"><GraduationCap size={24} /> Early Learning Services</div>
        <div className="user-area"><span className="user-name">{user.firstName} {user.lastName}</span><div className="avatar">{user.firstName?.[0]?.toUpperCase()}</div><button className="btn btn-light logout-btn" onClick={logout}><LogOut size={16} /> Logout</button><button className="menu-btn" onClick={() => setMobileNav(v => !v)}>{mobileNav ? <X /> : <Menu />}</button></div>
      </header>
      <div className="layout">
        <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
          <div className="nav-title">Main Menu</div>
          <nav className="nav">
            {items.map(item => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? 'active' : ''} onClick={() => { setPage(item.id); setMobileNav(false); }}><Icon size={18} />{item.label}<ChevronRight size={14} className="nav-arrow" /></button>; })}
          </nav>
        </aside>
        <main className="main"><Page page={page} user={user} /></main>
      </div>
    </div>
  );
}
...
function Page({ page, user }) {
  ...
  const pages = {
    ...
    teachingManual: <SimplePage title="Teaching Manual" text={user.role === 'instructor' ? 'Manage the course outline and teaching manual for your students.' : 'Your instructor’s course outline and teaching manual will appear here.'} />,
    announcements: <SimplePage title="Announcements" text={user.role === 'instructor' ? 'Create and publish announcements for your students.' : 'Announcements from your instructors will appear here.'} />,
    forum: <SimplePage title="Forum" text={user.role === 'instructor' ? 'Create discussion topics and respond to students.' : 'Join course discussions and respond to your instructor.'} />,
    level: <SimplePage title="Level" text="Set the title and description students will see for the course level." />,
    units: <UnitsPage />,
...
function Dashboard({ user }) {
  return <>
    <h1 className="page-title">{user.role === 'instructor' ? 'Instructor' : 'Student'} Dashboard</h1>
    <p className="muted">Your course overview.</p>
    <div className="cards">
      <StatCard icon={<Layers />} title="Level Courses" value="0" text="No level has been set yet." />
      <StatCard icon={<BookOpen />} title="Units" value="0" text="No units have been selected." />
      <StatCard icon={<ClipboardCheck />} title="Assessment Methods" value="0" text="No assessment methods have been selected." />
    </div>
    <section className="section"><div className="section-head"><div><h2>Welcome to ELES LMS</h2><p className="muted">The dashboard will populate from your MySQL data as the LMS modules are connected.</p></div></div><div className="card info-box"><strong>Early Childhood Learning Management System</strong><p>Courses, units, teaching manuals, assessments, announcements and forum activity will be managed from this interface.</p></div></section>
  </>;
}

function StatCard({ icon, title, value, text }) {
  return <div className="card stat-card"><div className="muted stat-label">{icon}{title}</div><div className="stat">{value}</div><p className="muted">{text}</p></div>;
}

function SimplePage({ title, text }) {
  return <><h1 className="page-title">{title}</h1><p className="muted">{text}</p><section className="section"><div className="card empty">This module is next in the LMS build. The interface is now using the attached frontend structure, ready to be connected to its MySQL API.</div></section></>;
}

...
export default function ForumPage({ role = 'student', userId = null }) {
  ...
  async function loadCourses() {
    try {
      const data = instructor ? await api('/instructor/courses') : await api('/student/courses');
  ...
  async function createPost(e) {
    ...
    setError('');
    setMessage('');
    if (!form.title.trim() || !form.body.trim()) return setError('Topic title and message are required.');
    setSaving(true);
    try {
      await api('/forum/posts', {
        method: 'POST',
        body: JSON.stringify({ title: form.title.trim(), body: form.body.trim(), courseId: form.courseId || null })
      });
  ...
  async function createReply(e) {
    e.preventDefault();
    if (!selected || !reply.trim()) return setError('Reply message is required.');
    setSaving(true);
    setError('');
    setMessage('');
    try {
      await api(`/forum/posts/${selected.id}/replies`, { method: 'POST', body: JSON.stringify({ body: reply.trim() }) });
      setReply('');
  ...
  return <>
    ...
    <section className="section">
      ...
      {loading ? <div className="card empty">Loading discussions...</div> : posts.length === 0 ? <div className="card empty">No discussions yet. Start the first one above.</div> : <div className="cards">{posts.map(post => <article className="card" key={post.id}>
        ...
        <h3>{post.title}</h3><p style={{ whiteSpace: 'pre-wrap' }}>{post.body}</p>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {userId && Number(post.author_id) === Number(userId) && <button className="btn btn-danger" onClick={() => removePost(post.id)}><Trash2 size={16} /> Delete</button>}
        </div>
      </article>)}</div>}
    </section>
